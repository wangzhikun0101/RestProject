package com.westvalley.g.materiel.expansion;
...
public class ExpansionMasterData extends BaseAction {
	...
	public String execute(RequestInfo requestInfo) {
		...
		String reach = "";//95.reach 新增字段 21.03.23  =""
		String bgbm="";//96.报关编码
		String bgmc="";//97.报关名称
		...
		for (Property p : properties) {
			if ("sapxxfh".equals(p.getName())) {//1.SAP返回消息
				...
				xyly=p.getValue();
			}else if("kcdgc".equals(p.getName())){//20.工厂，2019/6/19
				gc=p.getValue();
			}else if("jwlh".equals(p.getName())){//21.旧物料号
				jwlh=p.getValue();
			}else if("sapwlbm".equals(p.getName())){//22.SAP物料编码，2019/6/19
				sapwlbm=p.getValue();
			...
			}else if("reach".equals(p.getName())){//95.reach
					reach=p.getValue();
		...
		try {
			...
			tabless.setRow(0);
			//1.集团
			tabless.setValue("MANDT", "800");// 	测试720 正式800	//2.物料号码，无搜索帮助-22
			...
			//20. reach 是否达标 21.03.23
			if("0".equals(reach)){
				reach="";
			}else if("1".equals(reach)){
				reach="X";
			}
			tabless.setValue("ZREACH", reach);
		...
		if("S".equals(leix)){//返回成功
			...
			//修改返回信息
			this.updateForm(requestInfo, wul+""+xinx, wul);
		}
